...
export class Show_RelacionesComponent implements OnInit {
  ...
  private rows2:RelationshipI[] = []

  exportColumns: any[]=[];
  selectedProducts: RelationshipI[]=[];
  ...
  getAllScale() {
    ...
    this.relationshipService.getList().subscribe((relationshipsApiFrom) => {
      this.relationships =relationshipsApiFrom.relationships
      this.rows2=[]
      if(relationshipsApiFrom.relationships != undefined){
        for (const key of relationshipsApiFrom.relationships) {
          this.rows2.push(
            {
              id:key.id,
              name:  key.name,
      ...
      content: [
        {
          columns: [
            ...
            {
              width: '*',
              text: `Relaciones`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
            }
          ],
  ...
  exportExcel() {
    ...
    import("xlsx").then(xlsx => {
        const worksheet = xlsx.utils.json_to_sheet(this.relationships);
        const workbook = { Sheets: { 'data': worksheet }, SheetNames: ['data'] };
        const excelBuffer: any = xlsx.write(workbook, { bookType: 'xlsx', type: 'array' });
        this.saveAsExcelFile(excelBuffer, "relationships");
    });
  }
