...
@Component({
  selector: 'app-show_gender',
  templateUrl: './show_gender.component.html',
  styleUrls: ['./show_gender.component.css']
})
export class Show_genderComponent implements OnInit {
  ...
  private rows2:GenderI[] = []

  exportColumns: any[]=[];
  selectedProducts: GenderI[]=[];
  constructor(
    ...
    getAllScale() {
      this.genderService.getList().subscribe((categoryGroupsApiFrom) => {
        this.genders =categoryGroupsApiFrom.genders
        this.rows2=[]
        if(categoryGroupsApiFrom.genders != undefined){
          for (const key of categoryGroupsApiFrom.genders) {
            this.rows2.push(
              {
                id:key.id,
                name:  key.name,
        ...
        content: [
          {
            columns: [
              ...
              {
                width: '*',
                text: `Generos`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
              }
            ],
