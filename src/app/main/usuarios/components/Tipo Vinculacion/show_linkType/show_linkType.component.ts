...
@Component({
  ...
  styleUrls: ['./show_linkType.component.css']
})
export class Show_linkTypeComponent implements OnInit {
  ...
  private rows2:LinkTypeI[] = []

  exportColumns: any[]=[];
  selectedProducts: LinkTypeI[]=[];
  constructor(
    ...
    getAllScale() {
      this.linkTypeService.getList().subscribe((categoryGroupsApiFrom) => {
        this.linkTypes =categoryGroupsApiFrom.linkTypes
        this.rows2=[]
        if(categoryGroupsApiFrom.linkTypes != undefined){
          for (const key of categoryGroupsApiFrom.linkTypes) {
            this.rows2.push(
              {
                id:key.id,
                name:  key.name,
        ...
        content: [
          {
            columns: [
              ...
              {
                width: '*',
                text: `Tipos de Vinculaciones de Profesores`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
              }
            ],
    ...
    exportExcel() {
      let array:any[] = [];
      for (const key of this.linkTypes) {
        array.push({ 
          id: key.id,
      ...
      import("xlsx").then(xlsx => {
          ...
          this.saveAsExcelFile(excelBuffer, "linkTypes");
      });
    }
