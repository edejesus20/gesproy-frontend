...
@Component({
  selector: 'app-show_TypeMember',
  templateUrl: './show_TypeMember.component.html',
  styleUrls: ['./show_TypeMember.component.css']
})
export class Show_TypeMemberComponent implements OnInit {

  public typeMembers:TypeMemberI[]=[]
  first = 0;
  ...
  rows = 1;
  cols: any[]=[];
  public rows2:TypeMemberI[] = []
  exportColumns: any[]=[];
  selectedProducts: TypeMemberI[]=[]; 
  constructor(
  ...
  getUsrRoles() {
    this.typeMemberService.getList().subscribe((rolesFromApi) => {
      ...
      this.rows2=[]
      if(rolesFromApi.typeMembers != undefined){
        for (const key of rolesFromApi.typeMembers) {
      ...
      content: [
        {
          columns: [
            ...
            {
              width: '*',
              text: `Todos Los Tipo Miembros de Proyectos`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
            }
          ],
  ...
  exportExcel() {
    ...
    for (const key of this.typeMembers) {
      array.push({ 
        id: key.id,
        Nombre:key.name,
      })
    ...
    import("xlsx").then(xlsx => {
        ...
        this.saveAsExcelFile(excelBuffer, "typeMembers");
    });
  }
