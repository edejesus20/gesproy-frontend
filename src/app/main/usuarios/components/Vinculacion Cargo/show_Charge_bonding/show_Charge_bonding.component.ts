...
@Component({
  selector: 'app-show_Charge_bonding',
  templateUrl: './show_Charge_bonding.component.html',
  styleUrls: ['./show_Charge_bonding.component.css']
})
export class Show_Charge_bondingComponent implements OnInit {
  ...
  private rows2:Charge_bondingI[] = []

  exportColumns: any[]=[];
  selectedProducts: Charge_bondingI[]=[];
  constructor(
    ...
    getAllScale() {
      this.charge_bondingService.getList().subscribe((categoryGroupsApiFrom) => {
        this.charge_bondings =categoryGroupsApiFrom.charge_bondings
        console.log(categoryGroupsApiFrom.charge_bondings)
        this.rows2=[]
        if(categoryGroupsApiFrom.charge_bondings != undefined){
          for (const key of categoryGroupsApiFrom.charge_bondings) {
            this.rows2.push(
              {
                id:key.id,
                name:  key.name,
    ...
    exportExcel() {
      ...
      import("xlsx").then(xlsx => {
          ...
          this.saveAsExcelFile(excelBuffer, "charge_bondings");
      });
    }
