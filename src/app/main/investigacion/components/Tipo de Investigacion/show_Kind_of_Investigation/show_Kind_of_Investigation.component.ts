...
@Component({
  selector: 'app-show_Kind_of_Investigation',
  templateUrl: './show_Kind_of_Investigation.component.html',
  styleUrls: ['./show_Kind_of_Investigation.component.css']
})
export class Show_Kind_of_InvestigationComponent implements OnInit {

  public kind_of_Investigations:Kind_of_InvestigationI[]=[]
  first = 0;
  ...
  rows = 1;
  cols: any[]=[];
  private rows2:Kind_of_InvestigationI[] = []
  exportColumns: any[]=[];
  selectedProducts: Kind_of_InvestigationI[]=[]; 
  constructor(
    ...
    getUsrRoles() {
      this.kind_of_InvestigationService.getList().subscribe((rolesFromApi) => {
        ...
        this.rows2=[]
        if(rolesFromApi.kind_of_Investigations != undefined){
          for (const key of rolesFromApi.kind_of_Investigations) {
    ...
    exportExcel() {
      ...
      for (const key of this.kind_of_Investigations) {
        array.push({ 
          id: key.id,
          Nombre:key.name,
        })
      ...
      import("xlsx").then(xlsx => {
          ...
          this.saveAsExcelFile(excelBuffer, "kind_of_Investigations");
      });
    }
          ...
          content: [
            {
              columns: [
                ...
                {
                  width: '*',
                  text: `Todos Los Tipos de Investigacion`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
                }
              ],
