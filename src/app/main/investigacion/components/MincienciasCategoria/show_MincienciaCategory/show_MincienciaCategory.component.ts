...
@Component({
  selector: 'app-show_MincienciaCategory',
  templateUrl: './show_MincienciaCategory.component.html',
  styleUrls: ['./show_MincienciaCategory.component.css']
})
export class Show_MincienciaCategoryComponent implements OnInit {
  ...
  private rows2:MincienciaCategoryI[] = []

  exportColumns: any[]=[];
  selectedProducts: MincienciaCategoryI[]=[];
  ...
  getAllScale() {
    this.mincienciaCategoryService.getList().subscribe((mincienciaCategorysApiFrom) => {
      ...
      if(mincienciaCategorysApiFrom.mincienciaCategorys != undefined){
        for (const key of mincienciaCategorysApiFrom.mincienciaCategorys) {
          this.rows2.push(
            {
              id:key.id,
              name:  key.name,
      ...
      content: [
        {
          columns: [
            ...
            {
              width: '*',
              text: `Categoria de Minciencias`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
            }
          ],
  ...
  exportExcel() {
    ...
    import("xlsx").then(xlsx => {
        ...
        this.saveAsExcelFile(excelBuffer, "mincienciaCategorys");
    });
  }
