...
@Component({
  selector: 'app-show_Thematic_axis',
  templateUrl: './show_Thematic_axis.component.html',
  styleUrls: ['./show_Thematic_axis.component.css']
})
export class Show_Thematic_axisComponent implements OnInit {
  ...
  private rows2:Thematic_axisI[] = []

  exportColumns: any[]=[];
  selectedProducts: Thematic_axisI[]=[];
  constructor(
  ...
  exportExcel() {
    ...
    for (const key of this.thematic_axiss) {
      array.push({ 
        id: key.id,
        Nombre_Completo:key.name,
      })
    ...
    import("xlsx").then(xlsx => {
        ...
        this.saveAsExcelFile(excelBuffer, "thematic_axiss");
    });
  }
