...
@Component({
  selector: 'app-show_Knowledge_area',
  templateUrl: './show_Knowledge_area.component.html',
  styleUrls: ['./show_Knowledge_area.component.css']
})
export class Show_Knowledge_areaComponent implements OnInit {
  ...
  private rows2:Knowledge_areaI[] = []

  exportColumns: any[]=[];
  selectedProducts: Knowledge_areaI[]=[];
  constructor(
      ...
      content: [
        {
          columns: [
            ...
            {
              width: '*',
              text: `Areas de Conocimiento de Grupos`, alignment: 'center', fontSize: 15 ,bold: true,margin: [ 0, 40, 0, 0 ]
            }
          ],
  ...
  exportExcel() {
    ...
    for (const key of this.knowledge_areas) {
      array.push({ 
        id: key.id,
        Nombre_Completo:key.name,
      })
    ...
    import("xlsx").then(xlsx => {
        ...
        this.saveAsExcelFile(excelBuffer, "knowledge_areas");
    });
  }
